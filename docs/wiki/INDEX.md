...
| `independence` | `softmax-likelihood` | delivered | The CTC bridge's remaining half, closed: `TheLikelihoodLens` opens on the promise `ProportionsConverge` spoke ("likelihood is next"); `TheLossThatTrains` multiplies the per-frame matrix under the conditional license quoted on screen ("the frames are independent given the input — the conditioning series said when") and sums the logs |
...
| `e-and-ln` | `softmax-likelihood` | delivered | `TurningTheDial` pays calculus/'s on-screen debt ("why e appears in every probability machine"): base 2 is exactly (4/7, 2/7, 1/7), b^z = e^(z ln b), "no base is forced — every base is e at another T; e is the convention because ln is the natural counter" |
