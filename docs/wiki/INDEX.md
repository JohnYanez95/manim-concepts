...
| `ctc-alignment` | `ctc-decoding` | delivered (beam-search strand; label-prior variant still promised) | The row's last standing strand, closed by the decoding series (plan 015): `TheModelHeardNothing` re-runs the alignment series' sum-vs-max construction in its deployment costume (greedy's 0.36 vs the pooled 0.64, the very failure `ManyPathsOneWord` planted); `SearchTheTranscripts` puts beam search over collapsed prefixes on screen with the merge drawn as converging streams; `TheTwoLedgers` lands the two-probabilities subtlety the Ideas bullet promised, and its final beat sets the unpruned ledgers digit for digit beside `TheForwardTrellis`' final column. What remains of the old row: the label-prior variant (loss-side), still promised in Ideas |
...
| `logarithms` | `ctc-decoding` | delivered | `TheLoopClosed` says it on screen: "production ledgers run in log space — the merge is the log-add the logarithms series taught, never a max" — `TheUnderflowCliff`'s log-add carried into the beam's ledgers, with max-instead-of-add named as the silent reversion to path search |
