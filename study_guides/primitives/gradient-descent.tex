% Primitive: gradient-descent — GUIDE-FIRST (plan 012 D-E): no parent
% series. The video road's training beats run "plain gradient descent"
% on screen without teaching it; those scenes are cited only in "Where
% this goes", as users of the concept. Slotted after derivative-toolkit:
% everything here leans backward on slope-as-function and the sign-change
% habit. The loss walk is plan 010's anchor-M trajectory (proposed
% anchors in anchors-proposals-guide-first.yaml, copied exactly from the
% plan and reproduced independently by answers/gradient_descent.py);
% problem answers are computed by that script.
...
vanishes at $-1$, $0$, $1$, a walk started anywhere to the right of
$0$ settles into the valley at $1$ — but a walk started \emph{exactly}
at $0$ never moves: it sits on the hilltop, gradient zero, perfectly
stalled, certifying nothing. A zero gradient is where the walk ends;
...
named on screen) run precisely the walk quoted above. Farther out,
descent has earned its own future video series — learning-rate
cliffs, valleys and hilltops, the walk watched live — and this
chapter is that series' seed.
\end{narrative}
