% Primitive: dynamic-programming — formerly GUIDE-FIRST (plan 012 D-E),
% now series-backed: plan 013 built `algorithms/dynamic_programming_manim.py`
% FROM this chapter (ADR 008's pipeline, book-to-screen), and the wiki's
% oldest promised row is delivered on screen by `TheTrellisWasAMemo`.
% The scene arc mirrors these pages: the fib tree and its fold, the
% lattice recounted, the trellis re-read, the state's price, and the
% signature mapped. Numbers trace to plan
...
into log-space to survive. And this chapter was a seed that
sprouted: the video road performed dynamic programming without naming
it until \emph{these pages} drafted the naming — the `algorithms/'
series now teaches it on screen, scene for scene the arc you just
read.
\end{narrative}
